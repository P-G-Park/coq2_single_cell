...
freq_table <- table(all$ident1, all$patient) %>% as.data.frame()
pal <- pal_igv(alpha = 0.8)(13)

freq_control <- freq_table[1:13,]
freq_control <- freq_control %>% mutate(
  fraction = Freq/sum(Freq),
  ymax = cumsum(fraction),
  ymin = c(0, head(ymax, n=-1)),
  labelposition = (ymax + ymin) / 2,
  label = str_c(Var1, '(n = ', Freq, ')')
)
freq_control[6, 7] <- 0.445
freq_control[7, 7] <- 0.50
freq_control[10, 7] <- 0.76
freq_control[11, 7] <- 0.795
...
ggplot(freq_control %>% dplyr::slice(-12), aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Var1)) +
  geom_rect() +
...
freq_patient <- freq_table[14:26,]
freq_patient <- freq_patient %>% mutate(
  fraction = Freq/sum(Freq),
  ymax = cumsum(fraction),
  ymin = c(0, head(ymax, n=-1)),
  labelposition = (ymax + ymin) / 2,
...
freq_patient[6, 7] <- 0.41
freq_patient[7, 7] <- 0.45
freq_patient[10, 7] <- 0.678
freq_patient[11, 7] <- 0.695

ggplot(freq_patient , aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Var1)) +
  geom_rect() +
  geom_label( x=3.5, aes(y=labelposition, label=label), size=4) +
  coord_polar("y", start = 0) +
